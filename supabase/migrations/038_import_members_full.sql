-- Migration 038: full-dump member import RPC (v2.0).
-- Redesigns import_members to accept the full member record — identity, the 5 capability flags,
-- and a `responsible_name` (the responsible member's full_name). Stays DESTRUCTIVE
-- (DELETE-ALL + INSERT) and SECURITY DEFINER, matching the app's CSV full-dump import.
--
-- Two passes inside one transaction:
--   Pass 1 — INSERT every row with its capability flags (responsible_id left NULL).
--   Pass 2 — resolve each row's `responsible_name` → responsible_id within the SAME ward:
--            set responsible_id + contact_via_responsible = true only when the name resolves to
--            EXACTLY ONE other member; empty / unresolved / ambiguous names stay
--            responsible_id = NULL and contact_via_responsible = false.
-- Apply at the v2 cutover (after migration 037, alongside a DB backup). Do NOT modify 037.
